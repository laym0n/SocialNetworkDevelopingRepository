package social.network.exceptions;

public class UserInBlackListException extends RuntimeException {
    public UserInBlackListException() {
    }

    public UserInBlackListException(String message) {
        super(message);
    }

    public UserInBlackListException(String message, Throwable cause) {
        super(message, cause);
    }

    public UserInBlackListException(Throwable cause) {
        super(cause);
    }

    public UserInBlackListException(String message, Throwable cause, boolean enableSuppression, boolean writableStackTrace) {
        super(message, cause, enableSuppression, writableStackTrace);
    }
}
